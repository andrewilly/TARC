...
#include <windows.h>
#include <fstream>
#include <cstdint>   // Necessario per uint64_t
#include <cstring>   // Necessario per memcmp
#include "engine.h"
#include "ui.h"
...
// Nuova funzione per trovare l'offset tramite l'footer a 8 byte
size_t find_archive_offset(const std::string& exe_path) {
    ...
    // Legge gli ultimi 8 byte del file
    uint64_t archive_offset = 0;
    f.seekg(-8, std::ios::end);
    f.read(reinterpret_cast<char*>(&archive_offset), 8);

    // Verifica che l'offset abbia senso e che contenga la firma "TRC2"
