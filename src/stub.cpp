...
#include <windows.h>
#include <fstream>
#include <cstdint>   // Aggiunto per uint64_t
#include <cstring>   // Aggiunto per memcmp
#include "engine.h"
#include "ui.h"
...
size_t find_archive_offset(const std::string& exe_path) {
    ...
    // Un file SFX valido deve essere grande almeno 8 byte (il footer)
    ...
    if (archive_offset > 0 && archive_offset < (static_cast<uint64_t>(file_size) - 8)) {
        ...
        // Verifica che in quel punto ci sia davvero la firma "TRC2"
        char magic[4] = {0};
...
int main(int argc, char* argv[]) {
    ...
    //if (offset > 0) {
    //    std::cout << "[DEBUG] Dimensione dati rilevati: " << (total_size - offset - 8) << " bytes\n" << Color::RESET;
    //}
    // ------------------
