...
Codec select_auto(const std::string& path, const char* sample_data = nullptr, size_t sample_sz = 0) {
    ...
    // Se il campionamento rivela entropia altissima, non comprimere
...
ChunkResult compress_chunk(Codec codec, ZSTD_CCtx* zctx, const void* src, size_t src_sz, void* dst, size_t dst_cap, int level) {
    ...
    if (codec == Codec::ZSTD) {
        ...
        c_sz = LZ4_compress_default((const char*)src, (char*)dst, src_sz, dst_cap);
        if (c_sz <= 0) return {false, 0};
    } else if (codec == Codec::LZMA) {
        size_t out_pos = 0;
        lzma_ret ret = lzma_easy_buffer_encode(level, LZMA_CHECK_CRC64, nullptr, (const uint8_t*)src, src_sz, (uint8_t*)dst, &out_pos, dst_cap);
...
TarcResult compress(const std::string& arch_path, const std::vector<std::string>& files, bool append, int level) {
    ...
    // Gestione Append e Retrocompatibilità
    ...
    for (const auto& fpath : files) {
        ...
        uint64_t ts = fs::last_write_time(fpath).time_since_epoch().count();
        uint64_t cur_sz = fs::file_size(fpath);

        // SMART UPDATE LOGIC
        if (existing_files.count(fpath)) {
            if (existing_files[fpath].meta.timestamp == ts && existing_files[fpath].meta.orig_size == cur_sz) {
                UI::print_warning("Aggiornato: " + fpath);
                continue;
            }
            // Se modificato, rimuoviamo la vecchia entry dalla TOC futura
        ...
        ChunkHeader stop = {0, 0};
        IO::write_bytes(out, &stop, sizeof(stop));
...
// Nota: Extract, List e RemoveFiles seguono la logica v1.03 ma leggono la TOC con IO::read_toc che gestisce la v1.04
TarcResult list(const std::string& arch_path) {
    TarcResult result;
    FILE* f = fopen(arch_path.c_str(), "rb");
    Header h; std::vector<FileEntry> toc;
    if (!IO::read_toc(f, h, toc)) { result.ok = false; return result; }
    
    UI::print_header_list(arch_path, toc.size());
    for (auto& fe : toc) {
...
TarcResult extract(const std::string& arch_path, bool test_only) {
    ...
    for (auto& fe : toc) {
        ...
        while(true) {
            ...
            else if (fe.meta.codec == (uint8_t)Codec::NONE) memcpy(d_buf.data(), c_buf.data(), ch.raw_size);
            // ... Altri codec ...
        ...
        UI::print_extract(fe.name, integrity);
        if (dest) fclose(dest);
