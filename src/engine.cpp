...
#include <queue>
#include <vector>
#include <fstream> // Necessario per ifstream
...
// Definiamo la struttura all'interno del namespace affinché sia visibile
...
static CompressedChunk worker_compress_mt(Codec codec, std::vector<char> src, int level) {
    ...
    if (codec == Codec::LZMA) {
        ...
        size_t out_pos = 0;
        // Nota: stream_buffer_encode è thread-safe se chiamata separatamente
        ...
        int z_lvl = (level >= 9) ? 22 : 3;
        size_t const c_sz = ZSTD_compress(res.data.data(), res.data.size(), src.data(), src.size(), z_lvl);
...
TarcResult compress(const std::string& archive_path, const std::vector<std::string>& files, bool append, int level) {
    ...
    IO::write_bytes(f, &end_mark, sizeof(end_mark));
...
// Stubs per le altre funzioni per far compilare tutto
